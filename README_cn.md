...
配置你的ip摄像头，并配置好rtsp流的URL。

这里本文在计算机上使用[OBS Studio](https://obsproject.com/download)搭配[RTSP server plugin](https://github.com/iamscottxu/obs-rtspserver)进行模拟。使用本地摄像头、窗口采集或者视频文件作为直播源，打开`工具-->RTSP 服务器`设置URL的端口和目录，如`rtsp://localhost:554/rtsp`，点击`启动`。
...
将AirBox的LAN链接网线，WAN口与计算机相连。然后在计算机端配置IP地址，以windows操作系统为例，打开`设置\网络和Internet\更改适配器选项`，点击`以太网——>属性`，手动设置IP地址为`192.168.150.2`，子网掩码`255.255.255.0`。连接成功后，AirBox的IP即是`192.168.150.1`。

使用ssh远程工具，连接Airbox。以Termius为例：`NEW HOST`--> IP or Hostname：`192.168.150.1`，Username：`linaro`，Password：`linaro`。
...
在本地计算机打开[http://192.168.150.1:5000/](http://192.168.150.1:5000/)可查看frigate管理页面。
...
本文将转换yolov8n的模型，先下载yolov8n的onnx模型：
...
建立工作目录`yolov8`，并把模型文件`yolov8n_320.onnx`和图片文件`image/bus.jpg`都放入`yolov8`目录中。
...
本文在Windows系统下，安装Ubuntu 20.04进行编译。（在Microsoft Store中搜索ubuntu20并下载安装）。

打开Ubuntu 20.04，可以看到GLIBC版本合适，python需要另装。
...
本地计算机在[算能官网](https://developer.sophgo.com/site/index/material/77/all.html)下载sophon sdk压缩包。本文采用v23.10.01版本。
...
解压后在sophon-sail_xxxxx目录下找到sophon-sail_3.7.0.tar.gz，在sophon-img_xxxxx目录下找到libsophon_soc_0.5.1_aarch64.tar.gz，在sophon-mw_xxxxx目录下找到sophon-mw-soc_0.7.3_aarch64.tar.gz。
...
生成的安装包`sophon_arm-3.7.0-py3-none-any.whl`在`/dist`目录下。
...
使用sftp将`sophon_arm-3.7.0-py3-none-any.whl`复制到Airbox，并移动到frigate docker的`/config`对应挂载目录。在docker中安装sophon-sail:
...
将前面转换好的bmodel放在docker的`/config/model_cache`文件夹下。基于friagte环境，使用sail库编写了推理程序[sophgo.py](https://github.com/wlc952/frigate_in_sophgo_bm1684x/raw/main/sophgo.py)。
